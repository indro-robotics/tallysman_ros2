...
class Gps(Node):
    def __init__(
        ...
        self.declare_parameter(
            "region", "us"
        )  # The region where antenna is present. Parameter only needed when use_corrections is True
        self.declare_parameter(
            "config_path", ""
        )  # The path where the corrections_config is placed. Parameter only needed when use_corrections is True
        ...
        self.ser = UbloxSerial(
            self.unique_id, self.baud_rate, self.mode, self.use_corrections
        )
        ...
        if self.mode == "Heading_Base":
            ...
            self.rtcm_publisher = self.create_publisher(
                RtcmMessage, "rtcm_corrections", 100
            )
            self.base_status_publisher = self.create_publisher(
                GnssSignalStatus, "base_gps_extended", 100
            )
            ...
            self.rtcm_subscriber = self.create_subscription(
                RtcmMessage, "rtcm_corrections", self.handle_rtcm_message, 100
            )
            # Publisher for location information. Taking location from rover since it is more accurate.
            self.gps_publisher = self.create_publisher(NavSatFix, "gps", 100)
            # Publisher for location information. Taking location from rover since it is more accurate.
            self.gps_status_publisher = self.create_publisher(
                GnssSignalStatus, "gps_extended", 100
            )
