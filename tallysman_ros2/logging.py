...
class SimplifiedLogger:
    ...
    def _log(self, message:str):
        file_name = self._name+'_{day}.txt'.format(day = datetime.datetime.now().strftime('%Y-%m-%d'))
        formatted_log_message = "{time}  {log_prefix}  {message}".format(time=(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')), log_prefix = self._log_prefix, message = message)
